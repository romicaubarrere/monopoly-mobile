...
void main() {
  ...
  test(
    'event shape cannot carry forbidden identity or private-state fields',
    () {
      final event = action();
  ...
  test(
    'same confirmed retry emits at most one logical success event',
    () async {
  ...
  test('unknown lost-ack can later reconcile to one success', () async {
    ...
    expect(sink.events.map((event) => event.parameters['result']), <Object?>[
      'unknown',
      'success',
    ]);
  });
