...
void main() {
  testWidgets('auction_exposes_confirmed_bid_leader_deadline_and_participants', (
    tester,
  ) async {
    ...
    expect(
      find.bySemanticsLabel(
        r'Oferta actual: $ 300. Lidera: Rival A.',
      ),
      findsOneWidget,
    );
    expect(
  ...
  testWidgets('pending_bid_freezes_conflicting_controls_and_keeps_confirmed_bid', (
    tester,
  ) async {
    final controller = TextEditingController(text: '350');
    addTearDown(controller.dispose);
    ...
    expect(
      tester.widget<OutlinedButton>(find.widgetWithText(OutlinedButton, 'Pasar')).onPressed,
      isNull,
    );
  ...
  testWidgets('rejected_bid_recomposes_from_confirmed_state_and_allows_new_input', (
    tester,
  ) async {
    var bidCalls = 0;
    final controller = TextEditingController(text: '350');
    ...
    final input = tester.widget<TextField>(find.byKey(const Key('auction-bid-input')));
  ...
  testWidgets('passed_player_cannot_reenter_the_same_auction', (tester) async {
    ...
    expect(
      find.text('Ya pasaste; no podés volver a pujar en esta subasta.'),
      findsOneWidget,
    );
  });
        ...
        home: Center(
          child: SizedBox(
            ...
            child: MediaQuery(
              ...
              child: AuctionSheet(
                propertyLabel: 'Propiedad sintética con nombre deliberadamente largo',
