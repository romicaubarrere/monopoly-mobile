import 'dart:convert';
import 'dart:io';

import 'package:board_backend_api/backend_api.dart';
import 'package:test/test.dart';

void main() {
  test('sends bearer out-of-band and decodes a public response', () async {
    final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
    late Map<String, Object?> receivedBody;
    late String? receivedAuthorization;
    server.listen((request) async {
      receivedAuthorization = request.headers.value(
        HttpHeaders.authorizationHeader,
      );
      receivedBody = jsonDecode(
        await utf8.decoder.bind(request).join(),
      ) as Map<String, Object?>;
      request.response.headers.contentType = ContentType.json;
      request.response.write(jsonEncode(<String, Object?>{'ok': true}));
      await request.response.close();
    });
    final transport = HttpAuthorityWireTransport(
      baseUri: Uri.parse('http://127.0.0.1:${server.port}'),
      idTokenProvider: () async => 'synthetic.id.token',
    );
    addTearDown(() async {
      transport.close(force: true);
      await server.close(force: true);
    });

    final response = await transport.sendCommand(<String, Object?>{
      'family': 'game',
      'command': const <String, Object?>{'commandId': 'cmd-1'},
    });

    expect(response, <String, Object?>{'ok': true});
    expect(receivedAuthorization, 'Bearer synthetic.id.token');
    expect(receivedBody, isNot(contains('authorization')));
    expect(receivedBody, isNot(contains('token')));
  });

  test('permits HTTP only for local emulator origins', () {
    expect(
      () => HttpAuthorityWireTransport(
        baseUri: Uri.parse('http://authority.example.test'),
        idTokenProvider: () async => 'token',
      ),
      throwsArgumentError,
    );
    final local = HttpAuthorityWireTransport(
      baseUri: Uri.parse('http://localhost:8080'),
      idTokenProvider: () async => 'token',
    );
    local.close(force: true);
  });

  test(
    'retries transient public game reads with capped backoff and resets',
    () async {
      final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      var requests = 0;
      server.listen((request) async {
        requests += 1;
        final unavailable = requests <= 3 || requests == 5;
        request.response
          ..statusCode = unavailable
              ? HttpStatus.serviceUnavailable
              : HttpStatus.ok
          ..headers.contentType = ContentType.json
          ..write(
            jsonEncode(<String, Object?>{
              if (unavailable) 'error': 'temporarily unavailable',
              if (!unavailable) 'version': requests == 4 ? 1 : 2,
            }),
          );
        await request.response.close();
      });
      final delays = <Duration>[];
      final transport = HttpAuthorityWireTransport(
        baseUri: Uri.parse('http://127.0.0.1:${server.port}'),
        idTokenProvider: () async => 'synthetic.id.token',
        snapshotPollInterval: const Duration(milliseconds: 10),
        snapshotRetryMaxDelay: const Duration(milliseconds: 25),
        snapshotPollDelay: (delay) async => delays.add(delay),
      );
      addTearDown(() async {
        transport.close(force: true);
        await server.close(force: true);
      });

      final snapshots = await transport
          .watchPublicGame('game-1')
          .take(2)
          .toList();

      expect(snapshots, <Map<String, Object?>>[
        const <String, Object?>{'version': 1},
        const <String, Object?>{'version': 2},
      ]);
      expect(requests, 6);
      expect(delays, const <Duration>[
        Duration(milliseconds: 10),
        Duration(milliseconds: 20),
        Duration(milliseconds: 25),
        Duration(milliseconds: 10),
        Duration(milliseconds: 10),
      ]);
    },
  );

  test(
    'surfaces terminal public room polling errors without retrying',
    () async {
      final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      server.listen((request) async {
        request.response
          ..statusCode = HttpStatus.unauthorized
          ..headers.contentType = ContentType.json
          ..write(jsonEncode(<String, Object?>{'error': 'expired token'}));
        await request.response.close();
      });
      final delays = <Duration>[];
      final transport = HttpAuthorityWireTransport(
        baseUri: Uri.parse('http://127.0.0.1:${server.port}'),
        idTokenProvider: () async => 'synthetic.id.token',
        snapshotPollDelay: (delay) async => delays.add(delay),
      );
      addTearDown(() async {
        transport.close(force: true);
        await server.close(force: true);
      });

      await expectLater(
        transport.watchPublicRoom('room-1').first,
        throwsA(
          isA<AuthorityTransportException>().having(
            (error) => error.code,
            'code',
            'authenticationRejected',
          ),
        ),
      );

      expect(delays, isEmpty);
    },
  );

  test('retries a non-JSON 503 public snapshot response', () async {
    final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
    var requests = 0;
    server.listen((request) async {
      requests += 1;
      request.response.statusCode = requests == 1
          ? HttpStatus.serviceUnavailable
          : HttpStatus.ok;
      if (requests == 1) {
        request.response
          ..headers.contentType = ContentType('text', 'html')
          ..write('<html>temporarily unavailable</html>');
      } else {
        request.response
          ..headers.contentType = ContentType.json
          ..write(jsonEncode(<String, Object?>{'version': 1}));
      }
      await request.response.close();
    });
    final delays = <Duration>[];
    final transport = HttpAuthorityWireTransport(
      baseUri: Uri.parse('http://127.0.0.1:${server.port}'),
      idTokenProvider: () async => 'synthetic.id.token',
      snapshotPollInterval: const Duration(milliseconds: 10),
      snapshotPollDelay: (delay) async => delays.add(delay),
    );
    addTearDown(() async {
      transport.close(force: true);
      await server.close(force: true);
    });

    final snapshot = await transport.watchPublicGame('game-1').first;

    expect(snapshot, const <String, Object?>{'version': 1});
    expect(requests, 2);
    expect(delays, const <Duration>[Duration(milliseconds: 10)]);
  });

  test('requires retry cap not smaller than the polling interval', () {
    expect(
      () => HttpAuthorityWireTransport(
        baseUri: Uri.parse('http://localhost:8080'),
        idTokenProvider: () async => 'token',
        snapshotPollInterval: const Duration(seconds: 2),
        snapshotRetryMaxDelay: const Duration(seconds: 1),
      ),
      throwsArgumentError,
    );
  });
}
